...
public class MetodAux {
	...
	public static int menuOpciones(String titulo, String opciones[], String peticion) {
		...
		do {
			...
			if (!Main34.entrada.isEmpty() && checkSoloNumeroPositivoEntero()) {
	...
	public static boolean checkSoloNumeroPositivoEntero() {
		...
		for (int i = 0; i < Main34.entrada.length(); i++) {
			if (i == 0 && Main34.entrada.charAt(i) == '+')
				i++;
			if (!Character.isDigit(Main34.entrada.charAt(i)))
				esCorrecto = false;
		}
		if (!esCorrecto)
